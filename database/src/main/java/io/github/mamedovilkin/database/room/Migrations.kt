...
val MIGRATION_3_4 = object : Migration(3, 4) {
    override fun migrate(db: SupportSQLiteDatabase) {
        db.execSQL("ALTER TABLE `tasks` ADD COLUMN `repeatType` TEXT NOT NULL DEFAULT 'ONE_TIME'")
        db.execSQL("ALTER TABLE `tasks` ADD COLUMN `repeatDaysOfWeek` TEXT NOT NULL DEFAULT ''")
    }
}
...
val MIGRATION_5_6 = object : Migration(5, 6) {
    override fun migrate(db: SupportSQLiteDatabase) {
        db.execSQL("ALTER TABLE `tasks` ADD COLUMN `priorityType` TEXT NOT NULL DEFAULT 'NONE'")
    }
}